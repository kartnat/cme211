...
""" Class Airfoil: This class computes the lift coffiecients and stagnation
points for a given airfoil geometry.  At a high-level it consumes files 
describing the pressure distribution and geometry of an airfoil and follows
a formulaic process to return those values."""

class Airfoil:
    def __init__(self, inputdir):
        """Initialization method, initializes instance of Airfoil class with
        attribute inputdir.  Inputdir is a pathname for the directory where
        all the files are housed. """
    ...
    def read_files(self, coordFile, angleFile):
        """This function takes the names of the coordinate and specific angle
        of attack files, and returns a dictionary combining both the data files
        in a way that may be consumed by the later methods (key is a line number,
        values is a list [x coord, ycoord, pressure coefficient] for panel
        starting with (x coord, y coord))"""
        dataDict = {}
        ...
        for line in coordFile:
            ...
            if not line[0] or not line[1]:
                #Runtime error if one (or both) of the coordinates is missing, indicating
                # malformed data
        ...
        for line in angleFile:
            line = line.split()
            if not line:
                #Runtime error if there is a missing pressure measurement (or there is an
                # unnecessary blank line in the file)
                ...
                lineNumb += 1
        if lineNuma != (lineNumb + 1):    
        #Runtime error if every panel doesn't have a pressure
        # coefficent, in the case that there are no missing lines in either file
            raise RuntimeError("Missing pressure coefficient data for one more panels")
        return dataDict

    def chord_length(self, dataDict):
        """This function, as the name implies, returns the chord length of the 
        airfoil.  Just looking at the xy.dat file, this value is approximately
        one, but the function computes it exactly.  The input is the data
        dictionary generated in the previous method."""
        chordDict = {} 
    ...
    def lift_coeff(self, dataDict, alpha, chordLength):
        """Takes the data dictionary, angle of attack, and chord length to 
        produce the corresponding lift coefficient"""
    ...
    def stagnation_points(self, dataDict):
        """This function computes the stagation points, and returns a tuple 
        consisting of (i) coordinates of the points, (x,y), and (ii) the 
        corresponding pressure"""
        stagDict = {}     
        #Creates dictionary with keys being the pressure coefficients and values 
        #being the average coordinates of the corresponding panel
    ...
    def __str__(self):
        """This function prints the angles of attack, lift coefficients, and 
        stagnation points/pressure measurements in the desired output format"""
        str = ''
        k=0
        fileList = ["alpha-3.0.dat", "alpha0.0.dat", "alpha+3.0.dat", "alpha+6.0.dat", "alpha+9.0.dat"]
        angleList = ["-3.00", "0.00", "+3.00", "+6.00", "+9.00"]
        #Calls the functions in the required order, for each angle of attack, 
        #and appends the printed output to a string which is finally returned 
